        ...
        tableExport: function (options) {
            ...
            if (defaults.type == 'csv' || defaults.type == 'txt') {
                ...
                $(el).find('thead').find('tr').each(function () {
                    ...
                    tdData = $.trim(tdData.toUpperCase());
                ...
                //output
                tdData = tdData.replace(new RegExp("%20", 'g'), " ");
                tdData = tdData.replace(new RegExp("%23", 'g'), "#");
                tdData = tdData.replace(new RegExp("%2C", 'g'), ",");
                tdData = tdData.replace(new RegExp("%7C", 'g'), "|");
                tdData = tdData.replace(new RegExp("%3A", 'g'), ":");
                tdData = tdData.replace(new RegExp("%24", 'g'), "$");
                tdData = tdData.replace(new RegExp("%28", 'g'), "(");
                tdData = tdData.replace(new RegExp("%29", 'g'), ")");
                tdData = tdData.replace(new RegExp("%26", 'g'), "&");
                tdData = tdData.replace(new RegExp("%0A", 'g'), "");
                tdData = tdData.replace(new RegExp("%09", 'g'), "");
                var base64data = "base64," + $.base64.encode(tdData);
                //window.open('data:application/'+defaults.type+';filename=exportData;' + base64data);
                download('data:text/plain;' + base64data, (defaults.fileName==undefined?'download':defaults.fileName+"")+"_"+getFormattedDate()+"." + defaults.type, 'text/plain');
                ...
                download('data:application/json;' + base64data, (defaults.fileName==undefined?'download':defaults.fileName+"")+"_"+getFormattedDate()+"." + defaults.type, 'application/json');
                    ...
                    $(this).filter(':visible').find('td').each(function (index, data) {
                        if ($(this).css('display') != 'none') {
                            if (defaults.ignoreColumn.indexOf(index) == -1) {
                                ...
                                else excel += "<td>" + parseString($(this)) + "</td>";
                            }
                        }
                        colCount++;
                    });
                ...
                excelFile += "</body>";
                excelFile += "</html>";
                excelFile = excelFile.replace(new RegExp("%20", 'g'), " ");
                excelFile = excelFile.replace(new RegExp("%2C", 'g'), ",");
                excelFile = excelFile.replace(new RegExp("%3A", 'g'), ":");
                excelFile = excelFile.replace(new RegExp("%23", 'g'), "#");
                excelFile = excelFile.replace(new RegExp("%7C", 'g'), "|");
                excelFile = excelFile.replace(new RegExp("%24", 'g'), "$");
                excelFile = excelFile.replace(new RegExp("%28", 'g'), "(");
                excelFile = excelFile.replace(new RegExp("%29", 'g'), ")");
                excelFile = excelFile.replace(new RegExp("%26", 'g'), "&");
                excelFile = excelFile.replace(new RegExp("%0A", 'g'), "");
                excelFile = excelFile.replace(new RegExp("%09", 'g'), "");
                ...
                switch (defaults.type) {
                    ...
                    case 'powerpoint':
                        ...
                        break;
                    case 'doc':
                        download('data:application/msword;' + base64data, (defaults.fileName==undefined?'download.doc':defaults.fileName+".doc"), 'application/msword');
                        break;
                }
            ...
            function parseString(data) {
                ...
                return content_data.split('%09')[0];
            }
        }
    });
})(jQuery);
...
function getFormattedDate() {
    var date = new Date();
    var str = date.getDate() + "-" + (date.getMonth() + 1) + "-" +date.getFullYear() + "_" +  date.getHours() + ":" + date.getMinutes();
    return str;
}